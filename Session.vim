...
vmap <NL> <Plug>SchleppDown
vmap  <Plug>SchleppUp
vmap  <Plug>SchleppRight
nnoremap  :NERDTreeFind<C-w_w>
onoremap  :NERDTreeFind<C-w_w>
...
xnoremap <silent> <Plug>(highlightedyank) :call highlightedyank#yank('x')
nnoremap <silent> <Plug>(highlightedyank) :call highlightedyank#yank('n')
...
noremap <F4> :Autoformat
map <F8> :tabnew
map <S-Tab> :tabnext
nnoremap <F6> w
map <F2> :vertical wincmd f R " Open the file under the cursor in a vertical split
...
set undolevels=700
set updatetime=800
set wildignore=*.pyc,*/_build/*,coverage,.cache,node_modules,dist,.DS_Store,.git,.vscode,.idea,.next,obj,tags
set wildmenu
...
silent only
silent tabonly
cd ~/Projects/go/src/github.com/janbaer/github-oauth-bridge
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +14 config.prod.json
badd +120 main.go
badd +27 vendor/github.com/stretchr/gomniauth/providers/github/github.go
badd +27 Gopkg.toml
argglobal
silent! argdel *
$argadd .
...
setlocal nowrap
setlocal wrapmargin=0
let s:l = 184 - ((42 * winheight(0) + 24) / 48)
